...
본 프로젝트는 VPN 터널의 모든 데이터 패킷을 TCP 재전송 세그먼트로 위장하여 전송하는 방식을 사용하고 있습니다. 클라이언트-서버 간 통신에서 동일한 TCP sequence number를 반복적으로 사용함으로써, 네트워크 관측 시점에서 해당 트래픽이 단일 TCP 세그먼트에 대한 재전송으로 식별됩니다.
...
graph LR
    subgraph Windows Client
        App[Application] <--> TUN["TUN (WinTun)\n10.8.0.2/24"]
        TUN <--> Npcap["Npcap (NDIS)"]
    end

    subgraph Linux Server
        Internet[Internet] <--> STUN["TUN (tun0)\n10.8.0.1/24"]
        STUN <--> Raw["raw socket"]
    end

    Npcap <-- "TCP (seq=N 고정)\nfake retransmission" --> Raw
```
...
sequenceDiagram
    ...
    Client->>Server: SYN
    Server->>Client: SYN-ACK (syncookie)
    Client->>Server: ACK (cookie 검증)
    Note right of Server: 세션 생성

    Client->>Server: 공개키 + PSH/ACK
    Server->>Client: 서버 공개키
    Note right of Server: ECDH 키 교환

    Client->>Server: 암호화된 검증 토큰
    Server->>Client: 암호화된 검증 응답
    Note right of Server: 상호 검증 완료

    Client<<->>Server: 암호화된 데이터 전송 (seq 고정, 재전송 위장)
```
